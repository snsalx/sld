...
export default function Canvas({slide, setSlide}: {slide: Slide, setSlide: (update: Slide) => void}) {
  ...
  function handleMouse(target: SlideObject, type: 'move' | 'resize', x: number, y: number) {
    ...
    const update: SlideObject = {
      ...target,
      geometry: {
        left: type === "move" ? x / box.width * 100 : target.geometry.left,
        top: type === "move" ? y / box.height * 100 : target.geometry.top,
        width: type === "resize" ? x / box.width * 100 : target.geometry.width,
        height: type === "resize" ? y / box.height * 100 : target.geometry.height,
        borderRadius: target.geometry.borderRadius,
      },
    }
...
export function ObjectComponent(props: SlideObject & {onMouse: (type: 'move' | 'resize', x: number, y: number) => void}) {
  ...
  function startTrackingMove() {
    const controller = new AbortController();
    window.addEventListener('mouseup', () => controller.abort());

    window.addEventListener('mousemove', (event) => {
      props.onMouse("move", event.clientX, event.clientY)
  ...
  return <div style={geometry} className="absolute bg-surface0 p-2">
    <button className="absolute w-16 h-16 bg-blue rounded-full flex justify-center items-center hover:bg-teal transition" style={{top: 0, left: 0, transform: "translate(-50%, -50%)"}}>
      <ViewfinderCircleIcon className="text-base rotate-45 size-8" onMouseDown={startTrackingMove} />
    </button>
    <button className="absolute w-16 h-16 bg-green rounded-full flex justify-center items-center hover:bg-teal transition" style={{bottom: 0, right: 0, transform: "translate(50%, 50%)"}}>
